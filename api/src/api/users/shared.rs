...
/// Resolve the request's session cookie to the user id it belongs to,
/// with `role_filter` applied on the user document (what the old SQL join's
/// role condition did). A missing cookie is 401 outright; Ok(None) means the
/// session is expired/gone or the user doesn't match the filter.
async fn session_user(
    db: &Database,
    headers: &HeaderMap,
    role_filter: mongodb::bson::Document,
) -> Result<Option<Uuid>, E> {
    ...
    let mut filter = doc! { "_id": &session.user_id };
    filter.extend(role_filter);
    let user = db
    ...
    Ok(user
        .is_some()
        .then(|| Uuid::parse_str(&session.user_id).ok())
        .flatten())
}

pub async fn require_admin(db: &Database, headers: &HeaderMap) -> Result<Uuid, E> {
    session_user(db, headers, doc! { "role": "Admin" })
        .await?
        .ok_or((StatusCode::FORBIDDEN, "Admin access required"))
}

pub async fn require_user(db: &Database, headers: &HeaderMap) -> Result<Uuid, E> {
    session_user(db, headers, doc! {})
        .await?
        .ok_or((StatusCode::UNAUTHORIZED, "Not authenticated"))
}

pub async fn require_verified_user(db: &Database, headers: &HeaderMap) -> Result<Uuid, E> {
    session_user(db, headers, doc! { "role": { "$in": ["User", "Admin"] } })
        .await?
        .ok_or((StatusCode::FORBIDDEN, "Account approval required"))
}
